...
export function mountSaleReviews(container,{entries,catalog={},connection={},accountName,scope,open=false,onToggle=()=>{},filters={},onFilter=()=>{}}){
  ...
  function paint(i){
    ...
    try{
      ...
      if(details.open&&!card.hidden){const width=Math.round(plot.getBoundingClientRect().width)||560,stamp=data.at+'|'+data.rows.length+'|'+width;if(plot.dataset.stamp!==stamp){plot.innerHTML=tradeChartSvg(data.rows,g,width);plot.dataset.stamp=stamp;}}
      readyIds.add(g.id);
  ...
  function paintAll(qk){
    ...
    const shown=groups.filter(g=>reviewMatches(g,filters)),ready=shown.filter(g=>readyIds.has(g.id)).length;
    container.querySelector('[data-review-status]').textContent=groups.length?`符合 ${shown.length} 筆；已備妥 ${ready}／${shown.length} 筆；已存的圖不需重新下載。`:'';
  }
  const queryInput=container.querySelector('[data-review-query]'),dateInput=container.querySelector('[data-review-date]');
  function applyFilters(){cards.forEach((card,i)=>{card.hidden=!reviewMatches(groups[i],filters);});const empty=container.querySelector('[data-review-empty]');if(empty)empty.hidden=!groups.length||cards.some(c=>!c.hidden);paintAll();}
  function filterInput(){filters={symbol:queryInput.value,date:dateInput.value};onFilter({...filters});applyFilters();}
  if(queryInput)queryInput.oninput=filterInput;if(dateInput)dateInput.onchange=filterInput;
  const clear=container.querySelector('[data-review-clear]');if(clear)clear.onclick=()=>{queryInput.value='';dateInput.value='';filterInput();};applyFilters();
