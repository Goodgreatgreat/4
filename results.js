import {totalReturn,calculate,performance,quoteKey,usable,today} from './book.js';
...
export function resultView(u){
  ...
  const contents=markets.map(m=>{
    ...
    const annual=performance(full,m,u.quotes),total=totalReturn(selectedEntries,m,u.quotes);
    let specific='';
    if(stock){
...
export function monthlySeries(entries,market,start,end){
const totals=new Map();let month=start.slice(0,7),last=end.slice(0,7);
while(month<=last){totals.set(month,0);const d=new Date(month+'-01T00:00:00Z');d.setUTCMonth(d.getUTCMonth()+1);month=d.toISOString().slice(0,7);}
for(const e of calculate(entries).results)if(e.market===market&&e.date>=start&&e.date<=end&&(e.kind==='sell'||e.kind==='cash'))totals.set(e.date.slice(0,7),(totals.get(e.date.slice(0,7))||0)+(e.kind==='sell'?e.pnl:e.amount));
return [...totals].map(([month,amount])=>({month,amount}));
}
